...
func _generate_water(noise: float, instance: int) -> float:
	var color: = Color(MapVars.COLOR_WATER.to_html()).linear_interpolate(MapVars.COLOR_WATER_INTERPOLATION,
...
func _generate_sand(noise: float, instance: int) -> void:
	var color: = Color(MapVars.COLOR_SAND.to_html()).linear_interpolate(MapVars.COLOR_SAND_INTERPOLATE,
		Tools.noise_float_lerp(noise) + 0.2)
	Terrain.multimesh.set_instance_color(instance, color)

func _generate_mountain(min_height: float, height: float, noise:float, instance: int) -> float:
	var color: = Color(MapVars.COLOR_MOUNTAIN.to_html()).linear_interpolate(MapVars.COLOR_MOUNTAIN_INTERPOLATION,
